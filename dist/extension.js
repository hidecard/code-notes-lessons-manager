...
class NotesProvider {
    ...
    getChildren(element) { const notes = this.visible(); if (!element)
        return [new GroupItem('Favorites', 'special', 'favorites'), new GroupItem('Archive', 'special', 'archive'), new GroupItem('Trash', 'special', 'trash'), ...Array.from(new Set(notes.map(n => n.category))).sort().map(c => new GroupItem(c, 'category', c))]; if (element instanceof GroupItem) {
        if (element.kind === 'special')
            return notes.filter(n => element.key === 'favorites' ? n.favorite : element.key === 'archive' ? n.archived : !!n.deletedAt).map(n => new NoteItem(n));
        if (element.kind === 'category') {
            const days = Array.from(new Set(notes.filter(n => n.category === element.key).map(n => n.day || 'Unsorted'))).sort();
            return days.map(d => new GroupItem(d, 'day', `${element.key}::${d}`));
        }
        if (element.kind === 'day') {
            const parts = element.key.split('::');
            return notes.filter(n => n.category === parts[0] && (n.day || 'Unsorted') === parts[1]).map(n => new NoteItem(n));
        }
    } return []; }
...
function activate(context) {
    ...
    const openNote = (note) => { const panel = vscode.window.createWebviewPanel('codeNoteEditor', note.title, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true }); let preview = false; const render = () => panel.webview.html = editorHtml(note, preview); render(); panel.webview.onDidReceiveMessage(async (m) => { if (m.type === 'save') {
        ...
        note.deletedAt = note.deletedAt ? undefined : new Date().toISOString();
        note.updatedAt = new Date().toISOString();
        store.upsert(note);
        provider.refresh();
        render();
