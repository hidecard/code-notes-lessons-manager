...
async function exportNotePdf(note) { const uri = await vscode.window.showSaveDialog({ defaultUri: vscode.Uri.file(`${note.title.replace(/[^a-z0-9-_]+/gi, '-').toLowerCase() || 'note'}.pdf`), filters: { PDF: ['pdf'] } }); if (!uri)
    ...
    if (/^#/.test(raw))
        ...
        for (const part of parts) {
            ...
            doc.fillColor(highlight ? '#7a5d00' : '#1f2937').font(bold ? 'Helvetica-Bold' : 'Helvetica').text(part.replace(/^\*\*|\*\*$/g, '').replace(/^==|==$/g, ''), { continued: true });
        }
        doc.text('');
    }
    doc.moveDown(.15);
